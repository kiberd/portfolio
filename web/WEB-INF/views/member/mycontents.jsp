        ...
        <form id="boardForm" name="boardForm" method="post">
            <table class="table table-striped table-hover">
                ...
                <c:forEach var="dto" items="${list}" varStatus="status">
                    <tr>
                        <td><c:out value="${dto.BId }"/></td>
                        <td><c:out value="${dto.BName }"/></td>
                        <td><c:forEach begin="1" end="${dto.BIndent}"> - </c:forEach>
                            <a href="/board/viewcontent${pageMaker.makeQuery(pageMaker.criteria.pageNum)}&bId=${dto.BId}">
                                ...
                                <c:out value="${dto.BTitle}"/></a>

                            <c:if test="${dto.ccount > 0}">
                                <c:out value="[${dto.ccount}]"/></a>
                            </c:if>

                        </td>
                        <td><c:out value="${dto.BDate}"/></td>
                        <td><c:out value="${dto.BHit}"/></td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
            ...
            <div class="col-md-12 col-xs-12 text-center">
                <ul class="pagination">
                    ...
                    <c:if test="${pageMaker.criteria.pageNum > 1}">
                        <li class="page-item">
                            <a class="page-link" href="${path}/member/myContents${pageMaker.makeQuery(pageMaker.startPage)}&memId=${memId}&count=${count}"
                               aria-label="Previous">
                    ...
                    <c:if test="${pageMaker.criteria.pageNum > 5}">
                        <li class="paginate_button">
                            <a href="${path}/member/myContents${pageMaker.makeQuery(1)}&memId=${memId}&count=${count}">1</a>
                        </li>
                    ...
                    <c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
                        <li class="paginate_button ${pageMaker.criteria.pageNum == num ? 'active' :''}">
                            <a href="${path}/member/myContents${pageMaker.makeQuery(num)}&memId=${memId}&count=${count}">${num}</a>
                        </li>
                    </c:forEach>
                    ...
                    <c:if test="${pageMaker.totalPage - pageMaker.criteria.pageNum > 5}">
                        ...
                        <li class="paginate_button">
                            <a href="${path}/member/myContents${pageMaker.makeQuery(pageMaker.totalPage)}&memId=${memId}&count=${count}">${pageMaker.totalPage}</a>
                        </li>
                    </c:if>
                    ...
                    <c:if test="${pageMaker.criteria.pageNum < pageMaker.totalPage}">
                        <li class="page-item">
                            <a class="page-link" href="${path}/member/myContents${pageMaker.makeQuery(pageMaker.endPage)}&memId=${memId}&count=${count}"
                               aria-label="Next">
