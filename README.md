...
* **트랜잭션** 
   ...
   - 동시에 복수의 테이블로 쿼리를 하는 경우 동시성(Concurrency) 문제가 발생할 수 있기 때문에
    서비스단의 DB에 접근하는 메소드에 @Transactional 어노테이션을 붙여줘서 데이터의 무결성을 보장
   
   - 댓글이 추가되거나 삭제되는 트랜잭션  (BoardServiceImpl -> reply)
     + 댓글을 수정하기 위해 댓글 테이블로 접근하는 쿼리
     + 원글의 댓글개수를 조정하기 위해 원글 테이블로 접근하는 쿼리

   - 게시글 등록시 트랜잭션 (BoardServiceImpl -> write)
     + 게시글을 등록하기 위해 게시글 테이블로 접근하는 쿼리
     + 게시글의 파일을 저장하기 위해 파일테이블로 접근하는 쿼리

   - 답글 등록시 트랜잭션 (CommentServiceImpl -> insertRe)
     + 게시글을 등록하기 위해 게시글 테이블로 접근하는 쿼리
     + 게시글의 파일을 저장하기 위해 파일테이블로 접근하는 쿼리

   - 게시글 조회시 트랜잭션 (BoardServiceImpl -> viewContent)
